...
Transfers at or above `TRANSFER_REVIEW_THRESHOLD` are created as `PENDING_REVIEW` without moving money. Admin approval re-locks both account rows, re-validates status and balance, posts the debit/credit ledger entries, and marks the transfer `COMPLETED`. Admin rejection records the decision without touching balances.
...
Rate limiting is strategy-based. Local/default runs use an in-memory token bucket; Docker/prod can use Redis with an atomic Lua script so limits work across multiple app instances.
